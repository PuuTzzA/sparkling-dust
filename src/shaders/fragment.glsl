...
vec3 sparkles(vec2 position){
    float scaleCanvas = 300.;
    float scaleDust = 1.;
    float amount = .04;
    int layers = 3;
    ...
    for(int i = 0; i < layers; i++) {
        ...
        // Noise Motion
        float perlinX = cnoise(vec3(pos * 2., time / 8000. + float(i) * 100.));
        float perlinY = cnoise(vec3(pos * 2., time / 8000. + float(i) * 1000.));
        pos.x += (perlinX - 0.5) * .01;
        pos.y += (perlinY - 0.5) * .01;
